...
begin
	case(lut_index)			  
		...
		10'd 36: lut_data <= {8'h78 , 24'h362201}; // 50/60Hz detection     50/60Hz 靝厜靉＄汗杩囨�?
		...
		10'd 56: lut_data <= {8'h78 , 24'h430030};// YUV 422,YUYV
		10'd 57: lut_data <= {8'h78 , 24'h501f00};// YUV422
		10'd 58: lut_data <= {8'h78 , 24'h440e00};
		10'd 59: lut_data <= {8'h78 , 24'h5000a7}; // Lenc on, raw gamma on, BPC on, WPC on, CIP on // AEC target    鑷姩靇濆厜鎺у埗
		...
		10'd 65: lut_data <= {8'h78 , 24'h3a1f14};// fast zone low// Lens correction for ?   �?滃㝔睛ュ�?
		...
		10'd127: lut_data <= {8'h78 , 24'h583dce};// lenc BR offset // AWB   鑷姩靧藉钩�?
		...
		10'd158: lut_data <= {8'h78 , 24'h519e38};// AWB control // Gamma    浼界帥靇茬嚎
		...
		10'd175: lut_data <= {8'h78 , 24'h54901d};// color matrix   鑹杒僵靭╅樀
		...
		10'd186: lut_data <= {8'h78 , 24'h538b98}; // sign[8:1] // UV adjust   UV鑹杒僵楗坞拰杴﹝皟靝
		...
		10'd193: lut_data <= {8'h78 , 24'h501d40};// enable manual offset of contrast// CIP  閿愬寲靜岄檷�?
		...
		// 10'd208: lut_data <= {8'h78 , 24'h303541};// PLL
		...
		10'd212: lut_data <= {8'h78 , 24'h382121};// Sensor mirror on, ISP mirror on, H binning on, enable Compression
...
//		10'd251: lut_data <= {8'h78 , 24'h503d80};            //reg_data<=24'h503d80; test pattern selection control, 80:color bar,00: test disable
//		10'd252: lut_data <= {8'h78 , 24'h474101};            //reg_data<=24'h47401; test pattern enable, Test pattern 8-bit	 
		// 10'd253: lut_data <= {8'h78 , 24'h3c0708}; // lightmeter 1 threshold[7:0]
		...
		// 10'd266: lut_data <= {8'h78 , 24'h380803}; // DVPHO     800
		// 10'd267: lut_data <= {8'h78 , 24'h380920}; // DVPHO   
		// 10'd268: lut_data <= {8'h78 , 24'h380a02}; // DVPVO     600
		// 10'd269: lut_data <= {8'h78 , 24'h380b58}; // DVPVO     
		...
		// 10'd290: lut_data <= {8'h78 , 24'h300200}; // reset JFIFO, SFIFO, JPG
		// 10'd291: lut_data <= {8'h78 , 24'h3006ff}; // enable clock of JPEG2x, JPEG
		// 10'd292: lut_data <= {8'h78 , 24'h471302}; // JPEG mode 2
