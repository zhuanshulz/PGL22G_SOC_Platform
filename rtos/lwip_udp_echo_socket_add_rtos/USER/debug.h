...
#define PRINT_DEBUG(fmt, args...) 	 do{(DEBUG_P("\r\n[DEBUG] >> "), DEBUG_P(fmt, ##args));}while(0)     
#else
#define PRINT_DEBUG(fmt, args...)	     
#endif

#if PRINT_ERR_ENABLE
#define PRINT_ERR(fmt, args...) 	 do{(DEBUG_P("\r\n[ERR] >> "), DEBUG_P(fmt, ##args));}while(0)     
#else
#define PRINT_ERR(fmt, args...)	       
#endif

#if PRINT_INFO_ENABLE
#define PRINT_INFO(fmt, args...) 	 do{(DEBUG_P("\r\n[INFO] >> "), DEBUG_P(fmt, ##args));}while(0)     
#else
...
/* 断言 Assert */
#define AssertCalled(char,int) 	DEBUG_P("\nError:%s,%d\r\n",char,int)
